...
export function ShellAplicacao() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${menuAberto ? 'sidebar--aberta' : ''}`}>
      ...
      <div className="sidebar__rodape"><span className="sidebar__status"><i />API + Supabase</span><small>Ambiente seguro</small></div>
    </aside>
    ...
    <div className="app-main">
      <header className="topbar">
        ...
        <div className="topbar__usuario"><div className="avatar" aria-hidden="true">{(usuario?.email || 'U').slice(0, 1).toUpperCase()}</div><div><strong>{usuario?.email || 'Usuário autenticado'}</strong><span>{projeto?.status ? `Projeto ${projeto.status.toLowerCase()}` : 'Sessão Supabase'}</span></div><button className="botao botao--texto" onClick={sair}><LogOut aria-hidden="true" />Sair</button></div>
      </header>
